...
def remote_symdir_exists?(dir_path)
  'true' == capture("if [ -h #{dir_path} ]; then echo 'true'; fi").strip
end

# set the permissions for files recurisvely from the starting directory (dir_path)
def set_perms_files(dir_path, perm = 644)
  run "find #{dir_path} -type f -print0 | xargs -0 chmod #{perm}"
end

def set_touch_files(dir_path)
  run "find #{dir_path} -type f -exec touch {} \\;"
end

def set_touch_dirs(dir_path)
  run "find #{dir_path} -type d -exec touch {} \\;"
end

# set the permissions for directories recurisvely from the starting directory (dir_path)
def set_perms_dirs(dir_path, perm = 755)
  run "find #{dir_path} -type d -print0 | xargs -0 chmod #{perm}" if File.directory?(dir_path)
end